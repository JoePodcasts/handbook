...
The easiest way to start out is to [remove word gaps](https://help.descript.com/hc/en-us/articles/10164807277453-Adjusting-word-gaps-in-bulk). I do this for any gaps greater than 1 second. This ensures that any times the instructor paused to gather their thoughts etc, those blank spaces are removed. Leave 0.25 seconds gaps between clips where the instructor is on camera and clips where they're showing their screen.

Where appropriate, use stock videos/photos on Descript to highlight a concept.

You can zoom into sections of the screen on Descript. For example, in a video showing the pieces of RStudio, it may make sense to zoom into each one as I discuss it. Make zooms quick - around 1 second. Use this sparingly as it takes a lot of time! 

When there are two back-to-back clips of the speaker on camera, do a 0.5 second crossfade so the jump isn’t too abrupt.

Where appropariate, use callout boxes, arrows, lines, and/or text to draw the attention of the learner to a specific part of the screen (for example, when the insturctor says "you can see here"). Use color code ff7400 consistently when adding these attention grabbers.
